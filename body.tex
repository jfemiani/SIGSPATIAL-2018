...
For thin structures such as sidewalks that are visible in \ac{VHR} imagery, even small discrepancies between the digitized pathways and the imagery can be distracting when used to visualize an \replaced{overland}{overlaid} map.  
...
However in \figref{fig:Method_comparison} we show the segmentation results of these methods, and note that  they struggle to constrain their results to ribbon-like shapes with smooth thickness and mid-line.
...
To illustrate this challenge we look at the example of using \ac{SLIC} to segment the image in \figref{fig:slic}. In regions where shadows exist, the \ac{SLIC} superpixels snap to shadow boundaries instead of the target boundaries. 
\begin{figure}
    \centering
